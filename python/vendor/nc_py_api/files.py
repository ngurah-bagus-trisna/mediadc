...
_ND_ACCESS_LIMIT_CACHE = None


def _get_nd_access_limit():
    global _ND_ACCESS_LIMIT_CACHE
    if _ND_ACCESS_LIMIT_CACHE is None:
        _ND_ACCESS_LIMIT_CACHE = get_non_direct_access_filesize_limit()
    return _ND_ACCESS_LIMIT_CACHE


# Replaced by lazy accessor to avoid import-time DB query for missing cloud_py_api_settings table
...
def request_file_from_php(file_info: FsNodeInfo) -> bytes:
    if file_info["size"] >= _get_nd_access_limit():
        return b""

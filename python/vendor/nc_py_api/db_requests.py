...
def get_storages_info(num_id: Optional[int] = None) -> list:
    ...
    elif CONFIG["dbtype"] in ("sqlite3", "sqlite"):
        check_ext_mounts_query = f"SELECT name FROM sqlite_master WHERE type='table' AND name='{TABLES.ext_mounts}';"
    else:
    ...
    if num_id is None:
        query += " WHERE 1;" if CONFIG["dbtype"] == "mysql" else (" WHERE 1;" if CONFIG["dbtype"] in ("sqlite3", "sqlite") else ";")
    else:
